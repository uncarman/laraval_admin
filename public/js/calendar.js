  ...
  Calendar.prototype = {
    ...
    showCalendar: function () { // 输入数据并显示
      ...
      this.$calendarDate_item.each(function (i) {
        ...
        } else if(JSON.stringify(self.opts.markedDate) != '{}') {
            // 添加标记
            for(var i in self.opts.markedDate) {
                 if(self.opts.markedDate[i].indexOf(allDay_str) >=0) {
    ...
    inital: function () { // 初始化
      ...
      this.$calendarDate_item.hover(function () {
        ...
        self.$calendar_today.css({left: 0, top: 0}).hide();
          ...
          $(".item-selectedDay").removeClass("item-selectedDay");
          $(this).addClass("item-selectedDay");
          self.opts.clickFunc($(this).attr("data"));
      });
    },
